...
- 호위비 산정: NSR은 한국 자국 쇄빙선 '아라온'이 호위하므로 시장 수수료가 아니라 운영원가만
  반영된다(연료/경제성 결과의 escort_mode=own). NWP/TSR은 타국 쇄빙선이라 시장 수수료를 지불한다
  (escort_mode=foreign). 이 차이로 NSR의 부대비용이 NWP/TSR보다 크게 낮을 수 있으니 결론에 반영하라.
- 항해 소요일(transit_days)은 해빙 저항을 반영한 '실효속도' 기준이라, 화면의 명목속도(15노트) 표기
  소요일보다 다소 길 수 있다. 비용 산정은 이 실효 소요일을 따른다.
